...
<<<<<<< HEAD
        ...
        assertEquals(0, piece.position.id, "빽도 후 위치는 0이어야 합니다.");
        // 빽도 → 최종 이력 따라 1
        apply(YutResult.BACKDO.getStepCount());
        assertEquals(1, piece.position.id, "연속 빽도 후 위치는 1이어야 합니다.");
        // 빽도 → 0
        apply(YutResult.BACKDO.getStepCount());
        assertEquals(0, piece.position.id, "다시 빽도 후 위치는 0이어야 합니다.");
        // 개(2) → 마지막 완주
